...
const [updateTodos,{isLoading}]=useUpdateTodosMutation(_id)
    const toggleState = (id) => {
    
       const taskData = {isCompleted:!isCompleted}
    //    console.log(id,taskData);
       updateTodos({id:_id,data:taskData})
    }
    return (
        <div className="bg-white border rounded-md flex justify-around items-center p-3">
            <input className="mr-12 size-4" onClick={()=>toggleState(_id)} type="checkbox" name="complete" id="complete" />
